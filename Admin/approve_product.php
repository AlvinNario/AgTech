    ...
    try {
        ...
        if ($result->num_rows > 0) {
            ...
            if ($category_result->num_rows > 0) {
                ...
                $stmt = $conn->prepare("INSERT INTO productdetails (product_id, description, created_at) VALUES (?, ?, NOW())");
                if (!$stmt) throw new Exception("Prepare failed: " . $conn->error);
                $stmt->bind_param("is", $new_product_id, $product['product_description']);
                $stmt->execute();
